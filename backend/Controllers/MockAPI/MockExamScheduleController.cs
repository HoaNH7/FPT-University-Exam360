...
namespace backend.Controllers.MockAPI
{
    [ApiController]
    [Route("mockApi/[controller]")]
    public class MockExamScheduleController : ControllerBase
    {
        private static int currentIndex = 0;
        [HttpGet]
        public ActionResult<List<Schedule>> GetExamSchedule()
        {
            MockData mockData = new MockData(5, 5, 15, 1);
        ...
        public class MockData
        {
            ...
            public List<Schedule> GenerateSchedules()
            {
                ...
                for (int i = 0; i < _numOfSchedules; ++i)
                {
                    ...
                    List<ExamRoom> examRoomsForThisSchedule = examRooms.GetRange(i * _numOfExamRoomsPerSchedule, _numOfExamRoomsPerSchedule);
            ...
            public List<string> GenerateRoomNames()
            {
                ...
                return allPossibleRoomName.GetRange(0, _numOfPhysicalRoomAtCampus);
            }
            ...
            {
                const string slotJsonFilePath = "./Controllers/MockAPI/slot.json";
                return LoadJson<List<Slot>>(slotJsonFilePath);
            }

            public static List<Subject> GetSubjectsFromJson()
            {
                const string subjectJsonFilePath = "./Controllers/MockAPI/subject.json";
                return LoadJson<List<Subject>>(subjectJsonFilePath);
            }

            public static List<string> GetNamesFromJson()
            {
                const string nameJsonFilePath = "./Controllers/MockAPI/name.json";
            ...
            {
                const string imageUrlJsonFilePath = "./Controllers/MockAPI/image.json";
