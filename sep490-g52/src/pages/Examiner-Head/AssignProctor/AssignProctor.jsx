...
const AssignProctor = () => {
    ...
    const getStatus = (item) => {
        ...
        const startTime = moment(item.startTime, 'YYYY-MM-DD HH:mm');
        const endTime = moment(item.endTime, 'YYYY-MM-DD HH:mm');
    ...
    const getData = () => {
        ...
        if (searchDate) {
            queryString += `&date=${encodeURIComponent(searchDate)}`;
        }

        authfetch(
            BACKEND_URL + `/api/ExamRoom/ListAllRooms?${queryString}&pageNumber=${pageNumber}&pageSize=${pageSize}`,
        )
            .then((res) => res.json())
            .then((rs) => {
    ...
    //search, filter API
    const getDataAll = () => {
        authfetch(BACKEND_URL + `/api/ExamRoom/ListAllRooms?pageNumber=${pageNumber}`)
            .then((res) => res.json())
            .then((rs) => {
                const { totalCount, rooms } = rs;
                const newData = rooms.map((item) => ({
    ...
    const handleAssignProctor = () => {
        ...
        authfetch(BACKEND_URL + '/api/AssignProctor/ImportProctorIntoSchedule', {
            method: 'POST',
            headers: {
    ...
    const handleFileUpload = (e) => {
        ...
        reader.onload = (event) => {
            ...
            const processedData = worksheet.slice(1).map((row) => {
                const date = moment(new Date((row[0] - (25567 + 2)) * 86400 * 1000))
                    .subtract(7, 'hours')
                    .format('YYYY-MM-DD');
                const startTimeFull = moment(date + ' ' + row[2], 'YYYY-MM-DD HH:mm');
                const endTimeFull = moment(date + ' ' + row[3], 'YYYY-MM-DD HH:mm');
    ...
    const handleDownloadTemplate = () => {
        const templateData = [
            ['Date', 'Room', 'Start Time', 'End Time', 'Proctor Email'], // Headers
        ];
    ...
    const applyFilters = (updatedFilters = filters) => {
        ...
        // Filter by end date
        if (endDate) {
            filtered = filtered.filter((item) => new Date(item.endTime) <= endDate);
        }
    ...
    const handleExportExcelStudent = async () => {
        ...
        try {
            const response = await authfetch(
                BACKEND_URL + `/api/Statistic/ExportRoomListToExcel?fromDate=${fromDateString}&toDate=${endDateString}&subjectCode=${subjectFilterStudent1}&pageNumber=${pageNumber}&room=${roomFilter1}`,
            );
            ...
            link.href = url;
            link.setAttribute('download', 'Assign_Proctor.xlsx');
    ...
    return (
        <Fragment>
            <ToastContainer />
            <div className="main-container-assignproctor">
                <div className="assignproctor-management-header">
                    <Link className="assignproctor-management-link" to="/examinerhead">
                        Home
                    </Link>
                    {' > '}
                    <span style={{ fontSize: 14 }}> Assign Proctor</span>
                </div>
                <div className="body-container-assignproctor">
                            ...
                            {filterData && filterData.length > 0
                                ? filterData.map((item, index) => (
                                      <tr key={index}>
                                          ...
                                          <td data-title="Time">{item.startTime}</td>
                                          <td data-title="Time">{item.endTime}</td>
                                          <td data-title="Room No">{item.roomName}</td>
                                          <td data-title="Proctor Assigned">
                                              {item.proctorEmail ? <li>{item.proctorEmail}</li> : null}
                                          ...
                                          <td data-title="Semester">{item.semester}</td>
